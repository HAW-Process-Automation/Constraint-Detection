...
from .utils._sdl import get_workbook_id_from_url
from .utils._sdl import get_worksheet_url
...
class ConstraintDetection:
    ...
    def ConstraintDetectionUI(self, jupyter_notebook_url):
        ...
        self.AppBar = v.AppBar(color='#00695C', dense=True, dark=True, children=[
            v.ToolbarTitle(children=['Constraint Detection']), v.Spacer(), v.Divider(vertical=True), hamburger_menu])
        ...
        self.link_userguide = v.Btn(color='primary', x_small=True, icon=True, target='_blank',
                                    href='https://haw-process-automation.github.io/Constraint-Detection/userguide.html',
                                    attributes={"download": True}, children=[v.Icon(children=['mdi-open-in-new'])])
        ...
        #self.text_field_asset_tree = v.TextField(class_='', color='#00695C', v_model='', label='Asset Tree Name',
                                                 #hint='''Highest level of your asset tree e.g. for the Example asset
                                                 #tree that would be "Example"''')
        ...
        self.text_field_asset_tree_copy = v.TextField(class_='', color='#00695C', v_model='',
                                                      ...
                                                      hint='''The Add-on makes a copy of your asset tree. Give the copy
                                                       a name. By default, the word "Copy" will be appended to the 
                                                       asset tree name.''')

        self.explanation_head = v.ExpansionPanelHeader(class_="white--text", color='#00695C',
                                                       children=['What is the Contraint Detection Add-on used for?'])
        ...
        self.expansion_final = v.ExpansionPanels(children=[
            v.ExpansionPanel(children=[self.explanation_head, self.explanation_expl])])

        self.execute_button = v.Btn(color='#00695C', dark=True, block=True, elevation=2, children=['EXECUTE'],
                                    loading=False)
        ...
        self.export_button = v.Btn(color='#00695C', disabled=True, block=True, elevation=2,
